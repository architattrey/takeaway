    ...
    public function checkLogin($accessToken) {
      try{
        if (!empty($accessToken)) {
            ...
            $join  = [['table'=>'users b','condition'=>'a.user_id = b.user_id','type'=>'INNER']];
            
            $this->loginUser = $this->Common_model->fetch_using_join('a.access_token,b.user_id,b.name,b.status,b.user_type,b.lat,b.lng','users_login_session a',$join,['a.access_token'=>$accessToken],TRUE);
            
            if (!empty($this->loginUser)) {
                
                if ($this->loginUser->status != ACTIVE) {
                    
                    $this->response(['status_code' => USER_STATUS_NOT_ACTIVE,'status_message'=>$this->lang->line('USER_STATUS_NOT_ACTIVE')]);
                }
