...
class Vehicile_Model extends CI_Model {
    ...
    public function getVehicileList($params = array(), $limit = false, $offset = false) {//yha bydefault true jaa rha h driver m to iska mtlb registered vaale fetch honge or humcntrlr se 0 bhejenge to yha false get krega to pending vaale fetch honge
        try {

            $this->db->select();
            $this->db->from('driver_vehicile');
            
            //searching data
            if(isset($params['search']) && $params['search']!=''){
                $this->db->group_start();
                $this->db->like("vechile_name",$params['search']);
                $this->db->or_like("vechile_color",$params['search']);
                $this->db->or_like("vechile_model",$params['search']);
                $this->db->or_like("vechile_number",$params['search']);
                $this->db->group_end();
            }
            //using status filter
            ...
            $this->db->order_by("driver_vehicile.created_at","DESC");
     ...
     */
     ...
     public function fetchVehicileDetail($vehicileId=FALSE,$array=TRUE){
        try{
            $this->db->select('a.*,b.name,b.country_code,b.mobile,b.email,b.image');
            $this->db->from('driver_vehicile a');
            $this->db->join('drivers b','b.driver_id = a.driver_id' ,'LEFT');
            $this->db->where('a.vechile_id', $vehicileId);
