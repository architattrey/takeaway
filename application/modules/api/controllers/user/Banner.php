...
class Banner extends AutheticationLib {
    ...
    public function index_get(){
    	...
    	$banners  = $this->Common_model->fetch_data("banner","banner_id,CONCAT('".IMAGE_URL."',image_path) as banner_image",['limit'=>[10],'order_by'=>['created_at','DESC']]);
    	
    	if($banners){

    		$this->response(['status_code' => SUCCESS, 'status_message' => $this->lang->line('SUCCESS'),'data'=>['banners'=>$banners]]);	
    	}else{
    ...
    public function rechargeFriendWallet_post(){
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $this->setErrorJson(['user_id','credit','to_user_id']);
        }
