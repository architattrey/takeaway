...
class DriverSignup extends REST_Controller {
    ...
    public function signup_post() {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $this->setErrorJson(['email','mobile','password','name','country_code']);
        }
        // process singup for the next step, for inserting into db.
        $this->db->trans_begin();
        $driverId = $this->insertUserValues($postData,'');

        //inserting user session value.
        $this->insertSessionValues($driverId,$postData);
    ...
    private function insertUserValues($postData,$imageUrl){
        try{
            ...
            return $this->Common_model->insert_single('drivers',$savedata);
     ...
     *
     */
    private function insertSessionValues($driverId,$postData){
        try{
                $savedata = [
                'user_id' => $driverId,
            ...
            $this->Common_model->insert_single('drivers_login_session',$savedata);
    ...
    public function check_email_exists($key){
      ...
      $isExists  = $this->Common_model->fetch_data('drivers','email',['where'=>['email'=>$key],'where_in'=>['status'=>[ACTIVE,INACTIVE]]],true);
      if ($isExists){
    ...
    public function check_mobile_exists($key){
      ...
      $isExists  = $this->Common_model->fetch_data('drivers','mobile',['where'=>['mobile'=>$key,'country_code'=>$this->input->post('country_code')],'where_in'=>['status'=>[ACTIVE,INACTIVE]]],true);
      if ($isExists){
    ...
    private function sendRegistrationOtp($params) {
        ...
        $this->Common_model->update_single('drivers', ['otp' => $otp, 'otp_time' => time()], ['where' => ['driver_id' => $params['driver_id']]]);
    ...
    public function responseReturn($registerData) {
        ...
        $responseArr['created_at'] = $registerData['created_at'];
        return $responseArr;
    }
    ...
    public function vehicleResponseReturn($registerData) {
        ...
        if(isset($registerData['vechile_id']) && $registerData['vechile_id']!=""){
                ...
                $responseArr['vechile_name'] =   $registerData['vechile_name'];
                $responseArr['vechile_color'] =  $registerData['vechile_color'];
                $responseArr['vechile_number'] = $registerData['vechile_number'];
                $responseArr['vechile_type'] =   $registerData['vechile_type'];
                
                $responseArr['vechile_model'] = !empty($registerData['vechile_model']) ? $registerData['vechile_model'] : '';
                ...
                $responseArr['vechile_category'] =  $registerData['vechile_category'];
