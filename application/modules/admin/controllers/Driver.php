...
class Driver extends MY_Controller {
    ...
    public function index() {
        ...
        $this->data['driver'] = $this->Driver->getDriverList($this->params, $limit, $offset = ($page - 1) * $limit);
        

        $pageurl = 'admin/driver';
       
        $this->data["link"] = $this->Common_model->paginaton_link_custom($this->data['driver']['count'], $pageurl, $limit, $page);
        
        /* CSRF token */
        $this->data["csrfName"] = $this->security->get_csrf_token_name();
        $this->data["csrfToken"] = $this->security->get_csrf_hash();

        $this->loadAdminProfile('driver/registered', $this->data);
    }
     ...
     public function pending() {
        ...
        $this->data['Driver'] = $this->Driver->getDriverList($this->params, $limit, $offset = ($page - 1) * $limit,false);
        ...
        $this->loadAdminProfile('driver/pending', $this->data);
    }
    ...
    public function detail() {
        ...
        $driverId = (isset($get['id']) && !empty($get['id'])) ? $this->Common_model->decrypt($get['id']) : show_404();
        
        if (is_numeric($driverId) && $this->data['detail'] = $this->getDriverProfileDetail($driverId)) {
            
            $this->loadAdminProfile('driver/detail', $this->data);
        } else {

            show_404();
        }
    }
    public function getDriverProfileDetail($driverId)
    {
       return $this->Driver->fetchDriverDetail($driverId, true);
    }
    ...
    public function updateDriver()
   {
    try{
       ...
       // echo $DriverId;
        if ($req) {
                     $updateId =  $this->Common_model->update_single('drivers',['name'=>$req['name'],'email'=>$req['email'],'mobile'=>$req['mobile']],['where'=>['driver_id'=> $DriverId]]);
   ...
   /**
     ...
     */
     ...
     public function verified (){
        ...
        $driverId =  (isset($get['id']) && !empty($get['id'])) ? $this->Common_model->decrypt($get['id']) : show_404();
        $this->data['driver'] = $this->Driver->getVerified($driverId);
         if (is_numeric($driverId) && $this->data['driver']) {
            redirect(base_url().'Driver/pending');
             //$this->loadAdminProfile('driver/pending');
        } else {
